...
class LongestDistanceBetweenCompetitionsOnConsecutiveDaysUnique < Statistic
  def initialize
    @title = "Longest distance between competitions on consecutive days (unique)"
  ...
  def transform(query_results)
    query_results
      ...
      .filter_map do |person_link, results|
        ...
        next if best_from.nil?

        [
          person_link,
          max_distance.round,
          "[#{best_from['competition_name']}](https://www.worldcubeassociation.org/competitions/#{best_from['competition_id']}) (#{best_from['country_name']})",
          "[#{best_to['competition_name']}](https://www.worldcubeassociation.org/competitions/#{best_to['competition_id']}) (#{best_to['country_name']})"
        ]
      end
      .sort_by! { |row| -row[1] }
